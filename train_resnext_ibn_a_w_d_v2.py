...
import mxnet as mx
from symbol_resnext_ibn_a_w_d_v2 import resnext
...
def main():
    if args.data_type == "cifar10":
        ...
        units = per_unit*3
        symbol = resnext(units=units, num_stage=3, filter_list=filter_list, num_class=args.num_classes, num_group=args.num_group,
                        data_type="cifar10", drop_out=args.drop_out, ibn= True, bottle_neck = bottle_neck, bn_mom=args.bn_mom, workspace=args.workspace,
        ...
        symbol = resnext(units=units, num_stage=4, filter_list=[64, 256, 512, 1024, 2048] if args.depth >=50
                        else [64, 64, 128, 256, 512], num_class=args.num_classes, num_group=args.num_group, data_type="imagenet", drop_out=args.drop_out, ibn= True, bottle_neck = True
        ...
        if args.depth == 18:
            ...
            raise ValueError("no experiments done on detph {}, you can do it youself".format(args.depth))
        symbol = resnext(units=units, num_stage=4, filter_list=[64, 256, 512, 1024, 2048] if args.depth >=50
                        else [64, 64, 128, 256, 512], num_class=args.num_classes, num_group=args.num_group, data_type="vggface", drop_out=args.drop_out, ibn= True, bottle_neck = True
                        if args.depth >= 50 else False, bn_mom=args.bn_mom, workspace=args.workspace,
                        memonger=args.memonger)
    elif args.data_type == "msface":
        args.num_classes = 47003
        if args.depth == 18:
            units = [2, 2, 2, 2]
        elif args.depth == 34:
            units = [3, 4, 6, 3]
            ...
            raise ValueError("no experiments done on detph {}, you can do it youself".format(args.depth))
        symbol = resnext(units=units, num_stage=4, filter_list=[64, 256, 512, 1024, 2048] if args.depth >=50
                        else [64, 64, 128, 256, 512], num_class=args.num_classes, num_group=args.num_group, data_type="msface", drop_out=args.drop_out, ibn= True, bottle_neck = True
                        if args.depth >= 50 else False, bn_mom=args.bn_mom, workspace=args.workspace,
                        memonger=args.memonger)
